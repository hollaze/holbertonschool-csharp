...
class RoomObjects
{
    ...
    public static void IterateAction(List<Base> roomObjects, Type type)
    {
        foreach (var roomObject in roomObjects)
        {
            ...
            {
                case "IInteractive":
                    if (roomObject is IInteractive)
                    {
                        IInteractive interactive = (IInteractive)roomObject;
                        interactive.Interact();
                    }
                    break;

                case "IBreakable":
                    if (roomObject is IBreakable)
                    {
                        IBreakable breakable = (IBreakable)roomObject;
                        breakable.Break();
                    }
                    break;

                case "ICollectable":
                    if (roomObject is ICollectable)
                    {
                        ICollectable collectable = (ICollectable)roomObject;
                        collectable.Collect();
                    }
                    break;
            }
